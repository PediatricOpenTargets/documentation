...
```{r}
# install.packages(c('magrittr', 'dplyr', 'readr', 'gt', 'webshot2', 'DT'))
```
...
histologies <- read_tsv('../OpenPedCan-analysis/data/histologies.tsv')

### independent specimen lists
rna_primary <- read_tsv('../OpenPedCan-analysis/data/independent-specimens.rnaseqpanel.primary.eachcohort.tsv') 
rna_relapse <- read_tsv('../OpenPedCan-analysis/data/independent-specimens.rnaseqpanel.relapse.eachcohort.tsv')
dna_primary <- read_tsv('../OpenPedCan-analysis/data/independent-specimens.wgswxspanel.primary.eachcohort.prefer.wxs.tsv')
dna_relapse <- read_tsv('../OpenPedCan-analysis/data/independent-specimens.wgswxspanel.relapse.eachcohort.prefer.wxs.tsv')
```
...
# samples as DNA
         sequencing_type = 'DNA',
         experimental_strategy = ifelse(experimental_strategy %in% c('WGS', 'WXS'),
                                        'WGS/WXS', experimental_strategy)) %>% 
...
```{r}
dataset_disease_counts <- rbind(dna_counts, rna_counts) %>%
...
Filter table for data sets, data types, etc. not included in the current release.

```{r}
dataset_disease_counts_filtered <- dataset_disease_counts %>%
# TCGA and CHOP P30 DNA Panel data are not currently being used, so filter out
  filter(cohort != 'TCGA', !is.na(cancer_group),
         !(cohort == 'CHOP P30 Panel' & sequencing_type == 'DNA')) %>%
...
# Molecular Targets website and retain desired columns
  select(Dataset = cohort, `Sequencing Type` = sequencing_type,
        ...
        `Number of Tumors` = n) %>%
  arrange(Dataset, Disease, `Sequencing Type`, 
          `Tumor Stage`) 
```
...
Explorable table. You can use the search bar to filter any of the fields in the table, ex: searching "Rhabdomyosarcoma" will return all rows of the table with that disease. You can also copy the table or download it in CSV or Excel format by clicking the respective button.

```{r}
datatable(dataset_disease_counts_filtered, 
              extensions = 'Buttons',
...
```{r}
dataset_counts <- dataset_disease_counts %>%
...
# Molecular Targets website and retain desired columns
  select(Dataset = cohort, `Sequencing Type` = sequencing_type,
        `Tumor Stage` = stage, `Number of Tumors` = biospecimen_count) %>%
  arrange(Dataset, `Sequencing Type`, `Tumor Stage`) %>%
# TCGA and CHOP P30 Panel data are not currently being used, so filter out
  filter(Dataset != 'TCGA', 
         !(Dataset == 'CHOP P30 Panel' & `Sequencing Type` == 'DNA'))
```

Save as both a plain text downloadable table and as an image for insertion into the main repository README.
